...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    try {
      // Buscar por folio primero si parece un número
      const esNumero = /^\d+$/.test(q.trim())

      let organismoIds = []

      if (esNumero) {
        // Buscar contactos con ese folio y obtener sus organismos
        ...
        if (porFolio && porFolio.length > 0) {
          organismoIds = [...new Set(porFolio.map(c => c.unidades?.organismo_id).filter(Boolean))]
        }
      }
      ...
      if (organismoIds.length > 0) {
        query = query.in('id', organismoIds)
      } else {
        query = query.or(`rut.eq.${q},nombre.ilike.%${q}%`)
      }
